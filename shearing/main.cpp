...
void display() {
    ...
    float sheared[3][3];
    multiplyMatrices(shearing, vertices, sheared);

    // Draw sheared triangle in yellow
    glBegin(GL_TRIANGLES);
    glColor3f(1.0, 1.0, 0.0); // Yellow color for sheared triangle
    for (int i = 0; i < 3; i++) {
        glVertex2f(sheared[0][i], sheared[1][i]);
    }
    glEnd();
