...
void drawTranslatedCubeOutline(float r, float g, float b) {
    float vertices[8][4] = {
        {-0.5f, -0.5f, -0.5f, 1.0f},
        {0.5f, -0.5f, -0.5f, 1.0f},
        {0.5f, 0.5f, -0.5f, 1.0f},
    ...
    // Apply translation
    applyTransformation(vertices, 8, translationMatrix);
...
void drawScene() {
    ...
    // Eye position: (4, 3, 2), Center of view: (0, 0, 0), Up vector: (0, 1, 0)
    drawAxes();
    drawCubeOutline(0.0f, 0.0f, 1.0f); // Blue cube
    drawTranslatedCubeOutline(1.0f, 0.0f, 0.0f); // Red translated cube
    glutSwapBuffers();
}
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("3D Cubes with Axes, Colors, and Translation");
    initRendering();
